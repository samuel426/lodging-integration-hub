...
2026-09-04: POL-003 승인 전에는 [응답 정책 시나리오 S01~S16](search-response-policy.md)으로 대안을 검토합니다. 해당 테스트는 아직 작성하거나 실행하지 않았습니다.
...
- 유효한 관측 결과가 남는 개별 offer 정규화 실패는 HTTP 200, `partial=true`, 거절 건수를 반환하는 안을 검증합니다.
- 모든 offer가 잘못되고 정상 빈 결과도 없는 경우는 POL-003에서 502를 제안 중이며 승인 후 기대값을 확정합니다.
