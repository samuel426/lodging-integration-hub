...
사용자가 승인한 정책과 검토 중인 제안을 구분합니다. 정책 승인과 구현 시작 허가는 별개입니다. 현재는 문서 검토만 허용되었으며 도메인 구현, PR 생성과 병합을 진행하지 않습니다.
...
| POL-003 | 정규화 실패와 정상 빈 결과를 구분하는 검색 응답 | Proposed | 미승인 | 미구현 / 대안 검토 중 |
...
사용자는 가능한 시나리오, 각 근거, 최우선 추천을 요청했습니다. 특정 대안을 승인하지 않았으며 구현 시작을 명시적으로 보류했습니다.
...
| 2026-09-04 | 유효한 관측 결과가 있으면 부분 성공, 전혀 없으면 원인에 맞는 5xx | 현재 추천, 사용자 승인 대기 |

기존의 부분 성공 200 및 전체 외부 조회 실패 503이라는 상위 원칙은 승인되어 있습니다. 모든 데이터가 잘못된 경우 502를 사용하는 추천은 기존 전체 실패 503 규칙을 세분화하는 변경 제안이므로 추가 승인이 필요합니다.

### 검토 자료와 추적

- 대안, 추천, 시나리오와 HTTP 근거: [검색 응답 정책 검토](search-response-policy.md)
- 기존 초안: [ADR 0005](adr/0005-return-partial-search-results.md), [검색 API](api.md), [견고성 문서](resilience.md)
