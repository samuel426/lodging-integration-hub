...
선택한 이유, 발견한 반례와 수정 결과를 시간순으로 기록합니다. 각 날짜의 ‘후속 단계’와 보류 상태는 당시 기록입니다. 현재 완료 범위는 [구현 현황](docs/implementation-plan.md), 최신 검사 수치는 [품질 기록](docs/quality.md)을 기준으로 확인합니다.
...
## 2026-09-06 - Catalog 경계 검토와 필수 흐름 구현 재개

사용자가 기존 승인 정책에 따른 권장 순서의 진행을 요청했습니다. catalog 보완 뒤 availability adapter, 통합 검색, 관찰 가능성·품질 순서로 진행합니다. 선택 기능과 최종 main 병합은 해당 검토 시점에 결정합니다.

- 로컬 HTTP 재현에서 오류 상태 뒤 본문 절단이 분류되지 않은 예외를 전파하는 것을 확인했습니다. 본문 정리 중 알려진 전송 오류는 이미 확보한 HTTP 상태를 보존하고, 성공 본문의 읽기 timeout과 연결 중단은 각각 분류합니다. 예상하지 못한 내부 결함을 포괄적으로 숨기는 방식은 사용하지 않았습니다.
- 외부 상품 코드 중복과 JSON 객체 필드 중복은 다른 문제였습니다. 중복 `items`의 마지막 값이 빈 snapshot으로 반영될 위험을 확인해 parser 단계에서 거부하고 DB 보존 테스트를 추가했습니다.
- 서비스의 안전한 로그만으로 JDBC 상세값 비노출을 보장하지 못했습니다. Hibernate 실패 로그에서 fixture 저장값을 확인해 `logServerErrorDetail=false`를 설정하고 실제 DB 실패 로그를 검사합니다.
- 첫 검증은 Docker 엔진이 꺼져 Testcontainers 초기화에 실패했습니다. Docker를 기동하고 재검증합니다. 환경 실패를 코드 테스트 성공으로 계산하지 않습니다.
- Docker 기동 뒤 첫 B 본문 오류 계약 테스트가 예상 분류 대신 TIMEOUT이었고 후속 동일 계열 테스트들은 통과했습니다. cold JVM/container 비용을 계약 의미 검증과 분리하기 위해 해당 통합 suite의 읽기·전체 제한을 5초로 조정했습니다. 운영 기본값 2초는 유지하며 짧은 제한·본문 절단은 별도 실제 HTTP 경계 suite로 검증합니다. 기존 무응답(10초 지연)·drip(6초) 테스트는 5초 제한보다 길게 유지합니다.

완료된 실행 결과는 [테스트 기록](docs/testing.md)에 추가합니다.

## 2026-09-06 - 실시간 공급사 어댑터

- catalog PR #2를 검토 보완 후 dev에 squash merge하고 새 브랜치에서 실시간 조회를 구현했습니다.
- envelope 파싱과 개별 상품 변환을 분리해 정상 상품을 보존합니다. 명시적 빈 결과와 전부 거절된 결과는 다른 상태입니다.
- 자동 연결 재시도를 비활성화하고 query 인코딩, 50개 경계, 정수 overflow와 날짜 중복을 검증했습니다.
- 최초 어댑터 테스트는 통과했으나 PMD 명명/리터럴 검사에서 6건이 발견되어 수정했습니다.
...
- 정상 빈 catalog·빈 batch·검증 후 품절을 유효 관측으로 보존합니다. 전체 데이터 불능, mapping 불일치와 설정 오류를 승인된 우선순위로 판정합니다.
- S01~S16, 완료 순서 독립성, 공급사 전체 동시성 상한, Controller 입력/오류 계약을 테스트합니다. 원문 예외 메시지는 고객 응답과 오류 로그에 포함하지 않습니다.
- HTTP 오류 응답의 traceId 계약을 위해 Correlation ID filter를 이 단계에 포함했습니다. 비동기 공급사 전파와 운영 집계는 후속 관측 단계에서 완성합니다.
...
## 2026-09-07 - 설계 판단 정리와 Circuit Breaker

사용자가 설계 선택의 이유와 손실을 서비스 관점으로 설명하고 Circuit Breaker 하나를 완성하도록 요청했습니다. 반복 장애의 대기를 줄이되 정상 공급사 결과를 보존하는 목적에 범위를 맞췄습니다. 자동 재시도와 cache는 추가하지 않았습니다.

- README를 계약상 해석과 재량적 설계 판단으로 나눴습니다. 부분 성공에는 조회 범위 누락, 공급사별 상품 유지에는 중복 노출, 시작 시 catalog 동기화에는 변경 반영 지연이라는 손실을 연결했습니다.
- 기술을 먼저 고르고 이유를 붙이는 표현을 줄이고 문제·선택 기준·대안·손실 순서로 정리했습니다. 실제 사용자 조사나 운영 측정을 수행한 것으로 서술하지 않았습니다.
- 예외만 집계하는 Circuit Breaker 기본 동작으로는 결과값으로 전달되는 공급사 실패가 잡히지 않습니다. 일시적 이용 불가를 결과 predicate로 집계하고 인증·요청·데이터 오류는 성공으로도 세지 않도록 제외했습니다.
- HALF_OPEN에서 제외된 결과만 반복되면 복구 판정이 끝나지 않을 수 있어 최대 대기를 정했습니다. 취소 시 허가 반환과 별도 공급사 격리를 함께 검증했습니다.
- 전체 171건 실패·오류·skip 0, Spotless·PMD·build 통과. 실제 HTTP 요청 수로 차단 중 미호출을 검증했고 대기 후 복구 확인으로 CLOSED 전환을 확인했습니다.
- 차단 threshold는 mock에서 재현 가능한 초기값으로 명시했습니다. 운영 최적값이나 전체 검색 시간 보장을 주장하지 않습니다.
